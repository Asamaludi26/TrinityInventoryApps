...
export const ExportHandoverModal: React.FC<ExportHandoverModalProps> = ({
  ...
  useEffect(() => {
    ...
    const statusPart =
      statusFilter === "ALL"
        ? "ALL"
        : statusFilter === ItemStatus.COMPLETED
          ? "COMPLETED"
          : "PENDING";
        ...
        <div className="flex-1 p-6 overflow-y-auto space-y-5">
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
            <label className="flex items-center gap-2 text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">
              <BsCalendarRange className="w-3.5 h-3.5 text-primary-600" />
              Periode Data
            </label>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
            <label className="flex items-center gap-2 text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">
              <BsFunnel className="w-3.5 h-3.5 text-primary-600" />
              Status Dokumen
            </label>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
            <label className="flex items-center gap-2 text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-3">
              <BsFileEarmarkText className="w-3.5 h-3.5 text-primary-600" />
              Nama File (Preview)
            </label>
            <div className="flex items-center group focus-within:ring-2 focus-within:ring-primary-600/20 rounded-lg transition-all">
              <input
                type="text"
                value={customFilename}
                onChange={(e) => setCustomFilename(e.target.value)}
                className="block w-full px-4 py-3 text-xs font-mono font-medium text-slate-700 bg-slate-50 border-y border-l border-slate-300 rounded-l-lg focus:outline-none focus:border-primary-600 focus:bg-white transition-colors"
                placeholder="Masukkan nama file..."
              />
              <span className="px-4 py-3 text-xs font-bold text-slate-500 bg-slate-100 border border-slate-300 rounded-r-lg select-none">
                .csv
              </span>
            </div>
            <p className="text-[10px] text-slate-400 mt-2 ml-1">
              Nama file otomatis disesuaikan dengan filter yang dipilih.
            </p>
          </div>
          ...
          <div className="bg-slate-900 rounded-2xl overflow-hidden border border-slate-800 shadow-lg flex flex-col">
            <div className="px-5 py-3 bg-white/5 border-b border-white/5 flex justify-between items-center">
              ...
              <span
                className={`flex items-center gap-1.5 text-[10px] font-bold px-2 py-0.5 rounded-full ${filteredData.length > 0 ? "bg-emerald-500/20 text-emerald-400" : "bg-red-500/20 text-red-400"}`}
              >
                <BsInfoCircleFill className="w-3 h-3" />{" "}
                {filteredData.length > 0 ? "Ready to Export" : "No Data Found"}
              </span>
            </div>
                  ...
                  <div className="space-y-2 w-full">
                    <div className="flex justify-between text-[10px] font-medium text-slate-400">
                      <span className="flex items-center gap-1">
                        <BsCheckCircle className="text-emerald-500" /> Selesai
                      </span>
                      <span className="flex items-center gap-1">
                        <BsClockHistory className="text-amber-500" /> Proses
                      </span>
                    </div>
