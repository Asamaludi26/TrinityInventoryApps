...
export type ViewType = "overview" | "users" | "divisions";
...
export const useAccountsLogic = ({ currentUser }: UseAccountsLogicProps) => {
  ...
  const filteredUsers = useMemo(() => {
    return usersWithData
      ...
      .filter((user) =>
        userFilters.divisionId ? user.divisionId?.toString() === userFilters.divisionId : true
      );
  ...
  } = useSortableData<User & { assetCount: number }>(filteredUsers, {
    key: "name",
    direction: "ascending",
  });
    ...
    () =>
      divisions.map((division) => {
        ...
        const totalAssets = assets.filter(
          (a) => a.currentUser && memberNames.includes(a.currentUser)
        ).length;
        return {
          ...division,
  ...
  const filteredDivisions = useMemo(() => {
    return divisionsWithData.filter((d) =>
      d.name.toLowerCase().includes(divisionSearchQuery.toLowerCase())
    );
  ...
  } = useSortableData<Division & { memberCount: number; totalAssets: number }>(filteredDivisions, {
    key: "name",
    direction: "ascending",
  });
  ...
  const handleExport = () => {
    if (activeView === "users") {
      const dataToExport = sortedUsers.map((u) => ({
        ...u,
        divisionName: divisions.find((d) => d.id === u.divisionId)?.name || "N/A",
      }));
