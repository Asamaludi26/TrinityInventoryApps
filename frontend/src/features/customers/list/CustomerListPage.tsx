...
const CustomerListPage: React.FC<CustomerListPageProps> = ({
  ...
  useEffect(() => {
    if (initialFilters?.openEditFormFor) {
      setActivePage("customer-edit", {
        customerId: initialFilters.openEditFormFor,
      });
    }
  ...
  const filteredCustomers = useMemo(() => {
    return sortedCustomers
      ...
      .filter((c) => {
        ...
        const matchPackage = filters.servicePackage
          ? c.servicePackage === filters.servicePackage
          : true;
  ...
  return (
    <>
      <div className="p-4 sm:p-6 md:p-8">
        <div className="flex flex-col items-start justify-between gap-4 mb-6 md:flex-row md:items-center">
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              onClick={() => setActivePage("customer-new")}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex flex-wrap items-center gap-3">
                <span className="text-sm font-medium text-primary-600">
                  {selectedCustomerIds.length} pelanggan terpilih
                </span>
                ...
                {paginatedCustomers.map((customer) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* REFACTOR: Kolom Aset & Material yang dipisah */}
                      <td className="px-6 py-4 whitespace-nowrap align-top">
                        <div className="flex flex-col gap-1">
                          {/* Device Count (Recoverable) - Bold */}
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              onShowPreview({
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {" "}
                        <span
                          className={`px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusClass(customer.status)}`}
                        >
                          {" "}
                          {customer.status}{" "}
                        </span>{" "}
      ...
      <Modal
        ...
        title="Konfirmasi Hapus Pelanggan Massal"
        size="md"
      ...
      >
        ...
        <div className="flex items-center justify-end pt-5 mt-5 space-x-3 border-t">
          ...
          <button
            ...
            disabled={isLoading || deletableCustomersCount === 0}
            className="inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-danger rounded-lg shadow-sm hover:bg-red-700 disabled:bg-red-400 disabled:cursor-not-allowed"
          >
            {" "}
            {isLoading && <SpinnerIcon className="w-4 h-4 mr-2" />} Ya, Hapus (
            {deletableCustomersCount}) Pelanggan{" "}
          </button>
        </div>
      </Modal>
        ...
        footerContent={
          <>
            ...
            <button
              ...
              disabled={isLoading}
              className="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-lg shadow-sm hover:bg-primary-700"
            >
              {isLoading && <SpinnerIcon className="w-4 h-4 mr-2" />}Ubah Status
            </button>
          </>
        }
      >
