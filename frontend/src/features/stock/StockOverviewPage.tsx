...
const StockOverviewPage: React.FC<StockOverviewPageProps> = ({
  currentUser,
  setActivePage,
  onShowPreview,
  initialFilters,
  ...
  const { myAssets, loanedAssetDetails } = useMemo(() => {
    ...
    const finalLoanedAssetDetails = new Map<
      string,
      { returnDate: string | null; loanId: string }
    >();
      ...
      loan.items.forEach((item) => {
        ...
        assignedIds.forEach((assetId) => {
          if (!returnedIds.includes(assetId)) {
            finalLoanedAssetDetails.set(assetId, {
    ...
    return {
      myAssets: finalMyAssets,
      loanedAssetDetails: finalLoanedAssetDetails,
    };
  ...
  const { items: sortedMyAssets } = useSortableData(filteredMyAssets, {
    key: "name",
    direction: "ascending",
  });
    ...
    activeAssets.forEach((asset) => {
      ...
      if (!stockMap.has(key)) {
        ...
        stockMap.set(key, {
          ...
          isMeasurement,
          baseUnit: isMeasurement
            ? model?.baseUnitOfMeasure || type?.unitOfMeasure || "Meter"
            : undefined,
          storageBalance: 0,
          inUseBalance: 0,
          damagedBalance: 0,
    ...
    activeAssets.forEach((asset) => {
      ...
      if (current) {
        ...
        const currentContent =
          typeof asset.currentBalance === "number"
  ...
  const fullStockValue = summaryData
    ? `Rp ${summaryData.totalValueInStorage.toLocaleString("id-ID", { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`
    : "Rp 0";
  const shortStockValue = summaryData
    ? `Rp ${formatCurrencyShort(summaryData.totalValueInStorage)}`
    : "Rp 0";
  ...
  } = useSortableData(filteredStock, { key: "name", direction: "ascending" });
  ...
  const staffCategoryOptions = useMemo(
    () => [
      ...
      ...[...new Set(myAssets.map((a) => a.category))].map((c) => ({
        value: c,
        label: c,
      })),
    ],
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        {!isStaff && (
          <div className="flex bg-white dark:bg-slate-800 p-1 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm">
            ...
            >
              <BsBuilding
                className={`w-3.5 h-3.5 ${viewMode === "global" ? "text-white" : "text-gray-400"}`}
              />
              Gudang Utama
            </button>
            <button
            ...
            >
              <BsPersonBadge
                className={`w-3.5 h-3.5 ${viewMode === "personal" ? "text-white" : "text-gray-400"}`}
              />
              Stok Pribadi
            </button>
          </div>
          ...
          {staffSummary && (
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
              <SummaryCard
                ...
                onClick={() => {
                  setFilters((f) => ({ ...f, condition: "" }));
                  setTempFilters((f) => ({ ...f, condition: "" }));
                }}
              />
              <SummaryCard
                title="Kondisi Baik"
                value={staffSummary.goodCondition}
                icon={CheckIcon}
                color="green"
                isActive={filters.condition === "GOOD"}
                onClick={() => {
                  setFilters((f) => ({ ...f, condition: "GOOD" }));
                  setTempFilters((f) => ({ ...f, condition: "GOOD" }));
                }}
              />
              <SummaryCard
                title="Perlu Perhatian"
                value={staffSummary.needsAttention}
                icon={ExclamationTriangleIcon}
                color="amber"
                isActive={filters.condition === "ATTENTION"}
                onClick={() => {
                  setFilters((f) => ({ ...f, condition: "ATTENTION" }));
                  setTempFilters((f) => ({ ...f, condition: "ATTENTION" }));
                }}
              />
            </div>
          )}
            ...
            <div className="flex flex-col items-center justify-center h-full p-8 text-center text-gray-500 bg-white border-2 border-dashed rounded-xl">
              ...
              <p className="mt-1 text-sm">
                {searchQuery || Object.values(filters).some(Boolean)
                  ...
                  : "Aset yang Anda pegang akan muncul di sini. Jika Anda membutuhkan aset, silakan buat permintaan."}
              </p>
              <button
      ...
      {viewMode === "global" && summaryData && (
        <>
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            <SummaryCard
              title="Total Tipe Aset"
              value={summaryData.totalTypes}
              icon={AssetIcon}
              color="blue"
            />
            <SummaryCard
              ...
              <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-100 animate-fade-in-up mt-3">
                ...
                {filters.brand && (
                  <span className="inline-flex items-center gap-1 px-3 py-1 text-xs font-medium text-indigo-700 bg-indigo-50 border border-indigo-100 rounded-full">
                    ...
                    <button
                      ...
                      className="p-0.5 ml-1 rounded-full hover:bg-indigo-200 text-indigo-500"
                    >
                      <CloseIcon className="w-3 h-3" />
                    </button>
                  </span>
                ...
                {filters.outOfStockOnly && (
                  <span className="inline-flex items-center gap-1 px-3 py-1 text-xs font-medium text-red-700 bg-red-50 border border-red-100 rounded-full">
                    Stok Habis
                    <button
                      onClick={() => handleRemoveFilter("outOfStockOnly")}
                      className="p-0.5 ml-1 rounded-full hover:bg-red-200 text-red-500"
                    >
                      <CloseIcon className="w-3 h-3" />
                    </button>
                  </span>
