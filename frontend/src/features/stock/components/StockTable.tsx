        ...
        <tbody className="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700">
          {stockItems.length > 0 ? (
            stockItems.map((item) => {
              ...
              // --- DISPLAY UNIT LOGIC ---
              const displayUnit = isMeasurement
                ? item.baseUnit || "Meter"
              ...
              const defaultThreshold = isMeasurement
              ...
              const contentStockLevel = isMeasurement
                ? item.storageBalance
                : item.inStorage;
              const contentCapacityLevel = isMeasurement
              ...
              const isLowStock =
                !isOutOfStock && physicalCount <= activeThreshold;

              const storagePercentage =
                contentCapacityLevel > 0
                  ? (contentStockLevel / contentCapacityLevel) * 100
                  : 0;
              const barColorClass = isOutOfStock
                ? "bg-danger/80"
                : isLowStock
                  ? "bg-warning/80"
                  : "bg-success/80";
                ...
                >
                  ...
                  <td className="px-6 py-4 text-sm text-center whitespace-nowrap">
                    {editingThresholdKey === key ? (
                      ...
                      <div
                        onClick={() => {
                          setTempThreshold(String(activeThreshold));
                  ...
                  <td className="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">
                    <div className="flex items-center justify-end space-x-2">
                      ...
                      <button
                        ...
                        className={`inline-flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-white rounded-md shadow-sm transition-colors ${isOutOfStock || isLowStock ? "bg-amber-500 hover:bg-amber-600" : "bg-primary-500 hover:bg-primary-600"}`}
                      >
