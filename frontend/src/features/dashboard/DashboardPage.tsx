...
const formatCurrencyShort = (value: number): string => {
  if (value >= 1_000_000_000)
    return `${(value / 1_000_000_000).toLocaleString("id-ID", { minimumFractionDigits: 1, maximumFractionDigits: 1 })} M`;
  if (value >= 1_000_000)
    return `${(value / 1_000_000).toLocaleString("id-ID", { minimumFractionDigits: 0, maximumFractionDigits: 0 })} Jt`;
...
const UrgencyCard: React.FC<{
  ...
  subtext?: string;
  onClick?: () => void;
}> = ({ label, value, icon: Icon, color, subtext, onClick }) => {
  return (
    <div
      onClick={onClick}
      ...
      <div className="p-6 md:p-8 space-y-8 bg-gray-50/50 dark:bg-slate-950 min-h-screen">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div
            ...
            className="bg-blue-600 text-white p-8 rounded-3xl cursor-pointer hover:shadow-xl transition-all"
          >
            <h3 className="text-5xl font-bold">{myAssets.length}</h3>
            <p className="text-blue-100 mt-2">Aset Saya</p>
          </div>
          <div
    ...
    <div className="p-4 sm:p-6 md:p-8 space-y-8 bg-gray-50/30 dark:bg-slate-950 min-h-screen font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <FeatureStat
          ...
          items={[
            {
              label: "Internal (Teknisi)",
              value: featureMetrics.repairs.internal,
              color: "bg-blue-100 text-blue-700",
            },
            {
              label: "Vendor (Eksternal)",
              value: featureMetrics.repairs.vendor,
        ...
        <FeatureStat
          ...
          items={[
            {
              label: "Urgent / Darurat",
              value: featureMetrics.requests.urgent,
              color: "bg-red-100 text-red-700",
            },
            {
              label: "Project Based",
              value: featureMetrics.requests.project,
        ...
        <FeatureStat
          ...
          items={[
            {
              label: "Handover Internal",
              value: featureMetrics.movements.handovers,
              color: "bg-green-100 text-green-700",
            },
            {
              label: "Dismantle Pelanggan",
              value: featureMetrics.movements.dismantles,
