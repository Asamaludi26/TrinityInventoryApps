  ...
  return (
    ...
    >
      <div className="flex items-start gap-4 relative z-10">
        <div
          className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center border ${style.bg} ${style.text} ${style.border}`}
        >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-1">
            {item.priorityLabel ? (
              <div
                className={`flex items-center gap-1.5 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide rounded-md
                                ${
                                  item.priority === "high"
...
export const ActionableItemsList: React.FC<
  Omit<ActionableItemsListProps, "requests" | "assets">
> = ({ currentUser, setActivePage, onShowPreview }) => {
  const allActionableItems = useActionableItems(currentUser);

  const [activeTab, setActiveTab] = useState<
    "all" | "request" | "ops" | "damage"
  >("all");
  ...
  const filteredItems = allActionableItems.filter((item) => {
    ...
    if (activeTab === "request")
      return item.type === "request" || item.type === "asset_registration";
  ...
  const itemsToDisplay = isExpanded
    ? filteredItems
    : filteredItems.slice(0, INITIAL_ITEM_LIMIT);
  ...
  const requestCount = allActionableItems.filter(
    (i) => i.type === "request" || i.type === "asset_registration",
  ).length;
  const damageCount = allActionableItems.filter(
    (i) => i.type === "asset_damage",
  ).length;
  const opsCount = allActionableItems.filter((i) =>
    [
      "installation_pending",
      "maintenance_pending",
      "dismantle_pending",
  ...
  return (
    <div className="bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07)] dark:shadow-lg dark:shadow-black/20 overflow-hidden flex flex-col h-full">
      <div className="flex items-center justify-between px-6 py-5 border-b border-gray-50 dark:border-slate-700 bg-white dark:bg-slate-800">
        ...
        <span className="bg-primary-600 text-white text-xs font-bold px-2.5 py-1 rounded-lg shadow-sm">
                  ...
                  >
                    <span>
                      {isExpanded
                        ...
                        : `Tampilkan ${remainingItemsCount} lainnya`}
                    </span>
                    <ChevronDownIcon
