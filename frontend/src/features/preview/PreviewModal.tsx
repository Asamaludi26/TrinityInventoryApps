...
const PreviewModal: React.FC<PreviewModalProps> = (props) => {
      ...
      case "stockHistory": {
        ...
        return (
          <StockHistoryPreview
            ...
            title={getDisplayName(currentData)}
            onShowPreview={onShowPreview}
          />
        );
      }
  ...
  const renderFooter = () => {
    ...
    const canEdit =
      ...
      currentData &&
      ["asset", "customer"].includes(currentData.type);
        ...
        <div className="flex flex-col-reverse sm:flex-row sm:justify-between sm:items-center w-full gap-3">
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            {canEdit && (
              <button
                ...
                className="w-full sm:w-auto justify-center inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-gray-700 transition-all duration-200 bg-white border rounded-lg shadow-sm hover:bg-gray-50"
              >
          ...
          <div className="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-end gap-3 w-full sm:w-auto">
            {canReportDamage && (
              <button
                onClick={() => {
                  onReportDamage(asset);
                  handleClose();
                }}
                className="w-full sm:w-auto justify-center inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-amber-500 rounded-lg shadow-sm hover:bg-amber-600"
              >
            ...
            {isAdmin && asset.status === AssetStatus.DAMAGED && (
              <button
                ...
                className="w-full sm:w-auto justify-center inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-blue-600 rounded-lg shadow-sm hover:bg-blue-700"
              >
            ...
            {isAdmin && asset.status === AssetStatus.OUT_FOR_REPAIR && (
              <button
                onClick={() => {
                  onReceiveFromRepair(asset);
                  handleClose();
                }}
                className="w-full sm:w-auto justify-center inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-green-600 rounded-lg shadow-sm hover:bg-green-700"
              >
            ...
            {isAdmin && asset.status === AssetStatus.UNDER_REPAIR && (
              <div className="flex flex-col sm:flex-row gap-2 w-full">
                <button
                  onClick={() => {
                    onMarkAsRepaired(asset);
                    handleClose();
                  }}
                  className="w-full justify-center inline-flex items-center gap-2 px-3 py-2 text-sm font-semibold text-white bg-green-600 rounded-lg shadow-sm hover:bg-green-700"
                >
                ...
                <button
                  ...
                  className="w-full justify-center inline-flex items-center gap-2 px-3 py-2 text-sm font-semibold text-white bg-red-600 rounded-lg shadow-sm hover:bg-red-700"
                >
            ...
            {asset.status === AssetStatus.IN_STORAGE && (
              <>
                <button
                  onClick={() => {
                    onInitiateHandover(asset);
                    handleClose();
                  }}
                  className="w-full sm:w-auto justify-center inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white transition-colors bg-blue-600 rounded-lg shadow-sm hover:bg-blue-700"
                >
                ...
                {canBeInstalled ? (
                  <button
                    onClick={() => {
                      onInitiateInstallation(asset);
                      handleClose();
                    }}
                    className="w-full sm:w-auto justify-center inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white transition-colors bg-green-600 rounded-lg shadow-sm hover:bg-green-700"
                  >
                  ...
                  <Tooltip text="Kategori aset ini tidak dapat diinstal ke pelanggan.">
                    <div className="w-full sm:w-auto">
                      ...
                      <button
                        disabled
                        className="w-full justify-center inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-gray-400 rounded-lg cursor-not-allowed"
                      >
                        <CustomerIcon className="w-4 h-4" /> Pasang ke Pelanggan
                      </button>
                    </div>
      ...
      <div className="flex items-center justify-between w-full">
        ...
        {canEdit && (
          <button
            type="button"
            onClick={() => onEditItem(currentData)}
            className="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white transition-all duration-200 bg-primary-600 rounded-lg shadow-sm hover:bg-primary-700"
          >
            <PencilIcon className="w-4 h-4" /> Edit
          </button>
        )}
  ...
  return (
    ...
    >
      {history.length > 1 && (
        <nav
          className={`flex items-center text-sm font-medium text-gray-500 mb-4 ${currentData?.type === "asset" ? "px-6 -mt-2" : "-mt-2"}`}
