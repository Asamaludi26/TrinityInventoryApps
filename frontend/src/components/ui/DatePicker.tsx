...
const DatePicker: React.FC<DatePickerProps> = ({
  ...
  const changeYearRange = (offset: number) =>
    setYearRange((p) => [p[0] + offset, p[1] + offset]);
      ...
      <div className="grid grid-cols-3 gap-2 p-2">
        {months.map((month, i) => (
          <button
            ...
            className="p-3 text-center rounded-lg hover:bg-gray-100 font-semibold text-gray-700"
          >
            {month}
          </button>
        ))}
  ...
  let headerText = "";
  if (view === "days")
    headerText = new Intl.DateTimeFormat("id-ID", {
      month: "long",
      year: "numeric",
      ...
      <div className="mt-1 p-4 bg-white border border-gray-200 rounded-xl shadow-lg animate-zoom-in">
        <div className="flex items-center justify-between mb-4">
          <button
            type="button"
            onClick={() =>
              view === "days" ? changeMonth(-1) : changeYearRange(-12)
            }
          ...
          <button
            ...
            className="font-semibold text-base text-gray-800 hover:text-primary-600 transition-colors px-2 py-1 rounded-md"
          >
            {headerText}
          </button>
          <button
        ...
        <div className="pt-2 mt-2 border-t border-gray-200 flex justify-between">
          <button
            ...
            className="px-3 py-1.5 text-sm font-semibold text-primary-600 rounded-md hover:bg-primary-50"
          >
            Hari Ini
          </button>
          <button
            ...
            className="px-3 py-1.5 text-sm font-semibold text-gray-600 rounded-md hover:bg-gray-100"
          >
            Hapus
          </button>
        </div>
    ...
    <div className="relative">
      <div className="relative">
        <input
          ...
          disabled={disabled}
          className="block w-full pl-3 pr-10 py-2 text-gray-900 bg-gray-50 border border-gray-300 rounded-lg shadow-sm cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 sm:text-sm disabled:bg-gray-200/60 disabled:text-gray-500 disabled:cursor-not-allowed"
        />
