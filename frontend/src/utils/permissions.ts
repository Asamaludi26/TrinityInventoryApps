...
export const ALL_PERMISSIONS: {
  ...
  },
  {
    ...
    permissions: [
      ...
      { key: "categories:view", label: "Melihat kategori, tipe, & model aset" },
...
export const ALL_PERMISSION_KEYS = ALL_PERMISSIONS.flatMap((group) =>
...
export const ADMIN_LOGISTIK_PERMISSIONS: Permission[] = [
  ...
  // Request Flow - Admin Logistik bisa create request juga
  "requests:view:own",
  "requests:view:all",
  "requests:create",
  "requests:create:urgent",
...
export const ADMIN_PURCHASE_PERMISSIONS: Permission[] = [
  ...
  // Request Flow - Admin Purchase juga bisa create request
  "requests:view:own",
  "requests:view:all",
  "requests:create",
  "requests:create:urgent",
 ...
 *
 * Logic:
 * 1. Super Admin selalu punya semua akses
 * 2. Jika permission ada di ROLE_RESTRICTIONS untuk role user, tolak (security)
 * 3. Jika user.permissions memiliki permission tersebut, izinkan
 * 4. Fallback: Jika user.permissions kosong/tidak lengkap, gunakan ROLE_DEFAULT_PERMISSIONS
 */
export const hasPermission = (user: User | null, permission: Permission): boolean => {
  ...
  // Fallback to role default permissions if user.permissions is empty
  // Ini penting untuk user yang baru dibuat atau belum di-assign permissions
